<template>
  <div class="nuim-search-box" :class="{'nuim-search-fixed':isFixed}" :style="{top: isFixed ? top : '', position: fixPosition }">
    <div class="weui-search-bar" :class="{'weui-search-bar_focusing': !isCancel || currentValue}">
      <form class="weui-search-bar__form" @submit.prevent="$emit('on-submit', value)" action=".">
        <div class="nuim-search-mask" @click="touch" v-show="!isFixed && autoFixed"></div>
...
<script>
import uuidMixin from '../../libs/uuid'
