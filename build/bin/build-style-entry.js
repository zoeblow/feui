...
components.forEach(componentName => {
  ...
  const content = analyzeDependencies(componentName, libDir).map(
    component => `require('../../feui-css/${component}.css');`
  );
  fs.outputFileSync(
    path.join(libDir, componentName, "./style/index.js"),
    content.join("\n")
  );
});
...
function checkComponentHasStyle(componentName) {
  return fs.existsSync(
    path.join(__dirname, "../../lib/feui-css/", `${componentName}.css`)
  );
}
