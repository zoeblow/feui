...
components.forEach(componentName => {
  ...
  const content = analyzeDependencies(componentName, libDir).map(component => `require('../../feui-css/${component}.css');`);
...
function analyzeDependencies(componentName, libDir) {
  ...
  search(dependencyTree({
    ...
    filter: path => path.indexOf(`feui${SEP}lib${SEP}`) !== -1
  }), checkList);
...
function search(tree, checkList) {
  tree && Object.keys(tree).forEach(key => {
    ...
    const component = key.split(`${SEP}feui${SEP}lib${SEP}`)[1].replace(`${SEP}index.js`, '').replace(`mixins${SEP}`, '');
...
function checkComponentHasStyle(componentName) {
  return fs.existsSync(path.join(__dirname, '../../lib/feui-css/', `${componentName}.css`));
}
