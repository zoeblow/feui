...
gulp.task('icon-font', ['icon-font-ttf'], () => {
  const fontPath = path.resolve(__dirname, './icons/feui-icon.ttf');
  const hash = md5File.sync(fontPath).slice(0, 8);
  fs.renameSync(fontPath, path.resolve(__dirname, `./icons/feui-icon-${hash}.ttf`));

  let source = fs.readFileSync(path.resolve(__dirname, './icons/icon.css'), 'utf-8');
  source = source.replace('feui-icon.ttf', `feui-icon-${hash}.ttf`);

  fs.writeFileSync(path.resolve(__dirname, './src/icon.css'), source);
  shelljs.exec(`superman cdn /zanui/icon ./icons/feui-icon-${hash}.ttf`);
});
